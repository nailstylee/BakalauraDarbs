...
def capture_response_usage(response: Any, usage_totals: UsageTotals) -> None:
    """Collect Claude Messages API token usage for Excel/result summaries."""
    usage_totals.model_requests += 1
    usage = usage_attr(response, "usage")
    if usage is None:
        return

    uncached_input_tokens = usage_int(usage, "input_tokens")
    cache_creation_input_tokens = usage_int(usage, "cache_creation_input_tokens")
    cache_read_input_tokens = usage_int(usage, "cache_read_input_tokens")
    output_tokens = usage_int(usage, "output_tokens")

    # The agent does not enable prompt caching by default, so cache fields are
    # normally zero. input_tokens is stored as the total processed input token
    # count to stay comparable with the other model adapters.
    total_input_tokens = (
        uncached_input_tokens
        + cache_creation_input_tokens
        + cache_read_input_tokens
    )
    usage_totals.input_tokens += total_input_tokens
    usage_totals.cached_input_tokens += cache_read_input_tokens
    usage_totals.output_tokens += output_tokens
    usage_totals.total_tokens += total_input_tokens + output_tokens


def estimate_run_cost_usd(usage_totals: UsageTotals) -> float | None:
    """
    Optional Claude cost estimate.

    Provide these variables in .env to populate estimated_cost_usd:
      CLAUDE_INPUT_COST_PER_1M_USD
      CLAUDE_CACHED_INPUT_COST_PER_1M_USD
      CLAUDE_OUTPUT_COST_PER_1M_USD

    This agent does not enable Anthropic prompt caching by default. If caching
    is later added, cached-read tokens will use CLAUDE_CACHED_INPUT_COST_PER_1M_USD.
    """
    input_price = optional_float_env("CLAUDE_INPUT_COST_PER_1M_USD")
    output_price = optional_float_env("CLAUDE_OUTPUT_COST_PER_1M_USD")
    cached_input_price = optional_float_env("CLAUDE_CACHED_INPUT_COST_PER_1M_USD")
...
""".strip()
            ...
            "actions": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "x": {
                            "type": "number",
                            "description": "Click, wheel, or type_text x coordinate in screenshot pixels. Use 0 when unused.",
                        },
                        "y": {
                            "type": "number",
                            "description": "Click, wheel, or type_text y coordinate in screenshot pixels. Use 0 when unused.",
                        },
                        "from_x": {
                            "type": "number",
                            "description": "Drag start x coordinate in screenshot pixels. Use 0 when unused.",
                        },
                        "from_y": {
                            "type": "number",
                            "description": "Drag start y coordinate in screenshot pixels. Use 0 when unused.",
                        },
                        "to_x": {
                            "type": "number",
                            "description": "Drag end x coordinate in screenshot pixels. Use 0 when unused.",
                        },
                        "to_y": {
                            "type": "number",
                            "description": "Drag end y coordinate in screenshot pixels. Use 0 when unused.",
                        },
                        "wheel_direction": {
                            "type": "string",
                            "enum": ["up", "down", "none"],
                        ...
                        "wheel_steps": {
                            "type": "integer",
                            "description": "Number of mouse wheel notches from 0 to 120.",
                        },
                        "wait_ms": {
                            "type": "integer",
                            "description": "Wait duration in milliseconds from 0 to 5000.",
                        },
                        "text": {
                            "type": "string",
                    ...
                    "required": [
                        "action", "x", "y", "from_x", "from_y", "to_x", "to_y",
                        "wheel_direction", "wheel_steps", "wait_ms", "text", "clear_first",
                        "key", "purpose",
                    ],
                ...
                "content": [
                    {
                        ...
                        "source": {
                            ...
                            "data": encode_png_as_base64(screenshot_path),
                        },
                    },
    ...
    if not response_text:
        raise RuntimeError(
            f"Claude returned no text decision output. stop_reason={getattr(response, 'stop_reason', None)!r}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"Claude did not return valid JSON. Received: {response_text}"
        ) from exc
...
def solve_visual_puzzle() -> None:
    ...
    if not api_key:
        raise RuntimeError("ANTHROPIC_API_KEY was not found in .env.")
    ...
    max_output_tokens = int_env("CLAUDE_MAX_OUTPUT_TOKENS", 4096)
    ...
    try:
        with sync_playwright() as playwright:
            ...
            print(f"Claude max output tokens: {max_output_tokens}")
