...
def capture_response_usage(response: Any, usage_totals: UsageTotals) -> None:
    ...
    usage = usage_attr(response, "usage")
    if usage is None:
        return

    usage_totals.input_tokens += usage_int(usage, "input_tokens")
    usage_totals.output_tokens += usage_int(usage, "output_tokens")
    usage_totals.total_tokens += usage_int(usage, "total_tokens")

    input_details = usage_attr(usage, "input_tokens_details")
    output_details = usage_attr(usage, "output_tokens_details")
    usage_totals.cached_input_tokens += usage_int(input_details, "cached_tokens")
    usage_totals.reasoning_tokens += usage_int(output_details, "reasoning_tokens")


def estimate_run_cost_usd(usage_totals: UsageTotals) -> float | None:
    input_price = optional_float_env("OPENAI_INPUT_COST_PER_1M_USD")
    output_price = optional_float_env("OPENAI_OUTPUT_COST_PER_1M_USD")
    cached_input_price = optional_float_env("OPENAI_CACHED_INPUT_COST_PER_1M_USD")
...
def parse_model_json_object(response_text: str) -> dict[str, Any]:
    """Parse one model JSON object, tolerating accidental duplicated JSON output.

    Sometimes the model/API returns a string like `{...}{...}` where both
    objects are complete JSON objects. Plain json.loads raises "Extra data"
    for that. For this agent, the safest recovery is to use the first object
    and continue the run instead of crashing.
    """
    text = str(response_text or "").strip()
    if not text:
        raise RuntimeError("The model returned an empty response instead of JSON.")

    # Defensive cleanup in case a model ever wraps JSON in markdown despite the schema.
    if text.startswith("```"):
    ...
    try:
        ...
        if values and not trailing_text:
            if len(values) > 1:
                canonical_values = {
                ...
                if len(canonical_values) == 1:
                    print("Warning: model returned the same JSON object more than once. Using the first one.")
                else:
                    print("Warning: model returned multiple JSON objects. Using the first one.")

            parsed = values[0]
        else:
            raise RuntimeError(
                f"The model did not return valid JSON. Received: {response_text}"
            ) from original_exc

    if not isinstance(parsed, dict):
        raise RuntimeError(
            f"The model JSON must be an object. Received: {response_text}"
        )
...
- Return exactly one JSON object. Do not repeat the JSON object and do not append a second object.
...
Coordinate rules:
- All coordinates must be screenshot pixel coordinates.
- The origin is the top-left corner of the screenshot.
...
""".strip()
    ...
    response = client.responses.create(
        ...
        input=[
            {
                ...
                "content": [
                    ...
                    {
                        "type": "input_image",
                        "image_url": encode_png_as_data_url(screenshot_path),
                        "detail": image_detail,
                    },
                ],
            }
        ...
        text={
            "format": {
                "type": "json_schema",
                "name": "visual_puzzle_decision_round",
                "strict": True,
                "schema": {
                    "type": "object",
                    "properties": {
                        ...
                        "actions": {
                            ...
                            "items": {
                                ...
                                "properties": {
                                    ...
                                    "x": {
                                        "type": "number",
                                        "description": "Click or wheel x coordinate in screenshot pixels. Use 0 when unused.",
                                    },
                                    "y": {
                                        "type": "number",
                                        "description": "Click or wheel y coordinate in screenshot pixels. Use 0 when unused.",
                                    },
                                    "from_x": {
                                        "type": "number",
                                        "description": "Drag start x in screenshot pixels. Use 0 when unused.",
                                    },
                                    "from_y": {
                                        "type": "number",
                                        "description": "Drag start y in screenshot pixels. Use 0 when unused.",
                                    },
                                    "to_x": {
                                        "type": "number",
                                        "description": "Drag end x in screenshot pixels. Use 0 when unused.",
                                    },
                                    "to_y": {
                                        "type": "number",
                                        "description": "Drag end y in screenshot pixels. Use 0 when unused.",
                                    },
                                    "wheel_direction": {
                                        "type": "string",
                                        "enum": ["up", "down", "none"],
    ...
    raw = parse_model_json_object(response.output_text)
...
def solve_visual_puzzle() -> None:
    ...
    image_detail = os.getenv("IMAGE_DETAIL", "original").strip()
    if image_detail not in {"low", "high", "auto", "original"}:
        raise RuntimeError("IMAGE_DETAIL must be one of: low, high, auto, original.")
    ...
    try:
        with sync_playwright() as playwright:
            ...
            print(f"Image detail: {image_detail}")
